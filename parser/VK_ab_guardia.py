...
class VKABGuardiaParser(VkBaseParser):
    def __init__(self):
        super().__init__(source_name="ab_guardia")
        self.url = "https://api.vk.com/method/wall.get"  # URL VK API

    def parse(self):
        """Парсинг через VK API"""
        logging.info("Запуск парсера ВК ab_guardia")
        time.sleep(1)
        ...
        for post in data:
            try:
                ...
                news_item = NewsItem(
                    ...
                    link=link,
                    date=date,
                    source="ВК Агентство безопасности ГВАРДИЯ",
                )
